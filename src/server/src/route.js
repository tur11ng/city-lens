...
const {User, Activities, Activity, ActivityGeometry, ActivityProperties} = require("./model");
...
function encrypt(text, encryption_key) {
    encryption_key = Buffer.from(crypto.createHash('sha256').update(encryption_key).digest('base64').substr(0, 32));
...
adminRouter.get('/activities', authAdmin, function (req, res, next) {
    ...
    if (req.query.fromYear)
        conds.push({$gte: [{$year: "$activities.features.properties.timestamp"}, parseInt(req.query.fromYear, 10)]})

    if (req.query.toYear)
        conds.push({$lte: [{$year: "$activities.features.properties.timestamp"}, parseInt(req.query.toYear, 10)]});

    if (req.query.fromDOW)
        conds.push({$gte: [{$dayOfWeek: "$activities.features.properties.timestamp"}, parseInt(req.query.fromDOW, 10)]});

    if (req.query.toDOW)
        conds.push({$lte: [{$dayOfWeek: "$activities.features.properties.timestamp"}, parseInt(req.query.toDOW, 10)]});

    if (req.query.fromHour)
        conds.push({$gte: [{$hour: "$activities.features.properties.timestamp"}, parseInt(req.query.fromHour, 10)]});

    if (req.query.toHour)
        conds.push({$lte: [{$hour: "$activities.features.properties.timestamp"}, parseInt(req.query.toHour, 10)]});

    if (req.query.activitiesTypes)
        activitiesTypes = req.query.activitiesTypes;
    else
        activitiesTypes = ['STILL', 'ON_FOOT', 'WALKING', 'ON_BICYCLE', 'IN_VEHICLE', 'TILTING', 'EXITING_VEHICLE'];
...
userRouter.get('/eco-score/', authUser, function (req, res, next) {
    //TODO sort maybe?
    User.findOne({email: req.session.email}, {_id: false, "ecoScore": true}, (err, res1) => {
        if (err) {
            res.send(err);
            res.status(400);
        } else {
...
userRouter.get('/activities/last-upload', authUser, function (req, res, next) {
    User.findOne({email: req.session.email}, {_id: false, lastUpload: true}, (err, res1) => {
        if (err) {
            res.send(err);
            res.status(400);
        } else {
...
userRouter.get('/activities/type', authUser, function (req, res, next) {
    ...
    if (req.query.fromMonth)
        conds.push({$gte: [{month: "$activities.features.properties.timestamp"}, parseInt(req.query.fromMonth, 10)]});

    if (req.query.toMonth)
        conds.push({$lte: [{$month: "$activities.features.properties.timestamp"}, parseInt(req.query.toMonth, 10)]});
...
userRouter.get('/activities/most-records-by-dow-per-activity', authUser, function (req, res, next) {
    let conds = []
...
userRouter.get('/activities/most-records-by-hour-per-activity', authUser, function (req, res, next) {
    let conds = []
                    ...
                    if (err) {
                        ...
                        User.aggregate([
                            ...
                            {$unwind: "$activities.features"},
                            {
                                $addFields: {
                                    "ecoFriendly": {
                                        $in: ["$activities.features.properties.type", ["STILL", "ON_FOOT", "WALKING", "BICYCLE"]]
                                    }
                                }
                            },
                            {
