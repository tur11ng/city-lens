...
export class EcoScore extends Component {
    ...
    componentDidMount() {
        Api.ecoScore().then((res) => {
            console.log(res);
            this.setState({dataLabels: MONTHS, dataPoints: res.data.ecoScore.history, overall: res.data.ecoScore.overall});
        });
    }
    ...
    render() {
        ...
        return (<div>
            <h3>History</h3>
            <BarChart dataLabels={this.state.dataLabels} dataPoints={this.state.dataPoints} datasetLabel={this.state.datasetLabel} options={this.state.options}/>
