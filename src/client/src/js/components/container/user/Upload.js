    ...
    componentDidMount() {
        ...
        map.on('areaselected', (e) => {
            ...
            for(let i = markerWithin.features.length -1; i >= 0 ; i--){
                if(turf.booleanPointInPolygon(markerWithin.features[i].geometry.coordinates, polygon)){
    ...
    render() {
        return (
            <div className="text-center">
                <form onSubmit={this.onFormSubmit} className="custom-file justify-content-md-center mt-2">
                    <h1>Upload</h1>
                    <p>This is a community based project. It means that it is based on users voluntarily sharing their
                        data.
                        You can help keep the system updated by uploading your
                        own recent activity. If you don't know how to export your checkout FAQ</p>
                    <hr/>
                    <h3>1. Choose file</h3>
                    <p>Depending on the size of your file, your file will be sampled before upload.</p>
                    <input
                        id="file"
                        type="file"
                        name="selectedFile"
                    ...
                    <p>Using the lasso tool on the top right of the map you can select multiple areas to be excluded
                        from the data being uploaded. Depending on the size of your file, only a small (and hopefully
