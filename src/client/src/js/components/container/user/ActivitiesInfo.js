...
export class ActivitiesInfo extends Component {
    ...
    componentDidMount() {
        ...
        Api.activitiesInterval().then((res) => {
            console.log(res);
            this.setState({uploadIntervalFrom: res.data[0].start, uploadIntervalTo: res.data[0].end});
        });
    }
    ...
    render() {
        return (
            <div>
                ...
                <h5>Activities interval from {this.formatDate(this.state.uploadIntervalFrom)} to {this.formatDate(this.state.uploadIntervalTo)}</h5>
            </div>
        );
    }
}